...
(defn- subpattern-frame
  ...
  (assoc (initial-frame pattern eids multi? false) :attr attr))
...
#_(defn- conj-all-to-transient-set! [set values]
    (doseq [value values]
      (conj! set value)))
...
(defn- pull-attr-datoms
  ...
  (let [limit (get opts :limit +default-limit+)
    ...
    #_(println "\nparent pattern: " (:pattern parent))
    #_(println "\nattr: " attr)
    #_(println "\nfound: " found)
    
    (when found
      #_(conj-all-to-transient-set! (:datoms parent) found)
      (swap! (:result-datoms parent)
             into
             found))
...
(defn- pull-attr
  ...
  (let [[attr-key opts] spec]
    (if (= :db/id attr-key)
      (if (not-empty (d/datoms db :eavt [eid]))
        (conj (rest frames)
...
(defn- pull-wildcard-expand
  ...
  (let [datoms (group-by (fn [^Datom d] (.-a d)) (d/datoms db :eavt [eid]))
...
(defn- pull-pattern-frame
  [db [frame & frames]]
  #_(println "\npull-pattern-frame")
  #_(println "frame count" (+ 1 (count frames)))
  #_(println "pattern" (:pattern frame))
  #_(println "specs" (:specs frame))
  (if-let [eids (seq (:eids frame))]
    (if (:wildcard? frame)
      (pull-wildcard db
                     (assoc frame
...
(defn- pull-pattern
  [db frames]
  #_(println "\npull-pattern")
  #_(println "frame count" (count frames))
  #_(println "\nframes in pull-pattern " (count frames) frames)
  #_(println "state" (:state (first frames)))
  (case (:state (first frames))
    ...
    :done       (let [[f & remaining] frames
                      result (cond-> (persistent! (:results f))
                               (not (:multi? f)) first)
                      datoms (if-let [datoms (:result-datoms f)]
                               @datoms
                               #{})]
                  ...
                  #_(println "attern " (:pattern f))
                  #_(println "\nremaining " remaining)
                  #_(println "result datoms " datoms)
                  #_(println "result datoms in next frame " (:result-datoms (first remaining)))
                  (when (and (not (empty? datoms))
                             (:result-datoms (first remaining)))
                    (swap! (:result-datoms (first remaining)) into datoms))
...
#_(defn entid [db eid]
    (cond
      ...
      (cond
        ...
        nil
        :else
        (:e (first (d/datoms db :avet eid))))
      :else
      (raise "Expected number or lookup ref for entity id, got " eid
             {:error :entity-id/syntax
              :entity-id eid})))

#_(defn entid-strict [db eid]
    (or (entid db eid)
...
(defn pull-spec
  [db pattern eids multi? datoms?]
  (let [eids (into [] eids #_(map #(datascript-db/entid-strict db %)) #_eids)]
    (pull-pattern db (list (initial-frame pattern eids multi? datoms?)))))

(defn pull [db selector eid]
  (pull-spec db (dpp/parse-pull selector) [eid] false false))

(defn pull-datoms [db selector eid]
  (pull-spec db (dpp/parse-pull selector) [eid] false true))

(defn pull-many [db selector eids]
  (pull-spec db (dpp/parse-pull selector) eids true false))

#_(defn run-pull []
    #_(println "-----------------------")
    (let [db (datascript.core/db-with (datascript.core/empty-db agile-queue.server.handler/schema)
                                      [[:db/add 1 :queue/title "First queue"]
                                       [:db/add 1 :queue/first-task 2]
                                       [:db/add 2 :task/title "First task"]
                                       [:db/add 2 :task/next-task 3]
                                       [:db/add 3 :task/title "Second task"]])]
      (pull-datoms db [{:queue/first-task [:task/title]}
                       :queue/title]
                   1)))

;; curl -X POST "http://localhost:3001" -d "[:pull [{:queue/first-task [:task/title]} :queue/title] 1]"


(defn start []
  (let [uri (db/new-memory-db-uri)]
    ...
    (let [conn (d/connect uri)]
      ...
      #_(let [db (d/db conn)
              result (pull-datoms db [{:queue/first-task [:task/title]}
                                      :queue/title]
                                  0)]
